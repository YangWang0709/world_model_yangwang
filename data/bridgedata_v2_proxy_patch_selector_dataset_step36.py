"""Dataset helpers for Step36 proxy patch/token selector smoke training."""
...
from models.bridgedata_v2_proxy_patch_token_selector import proxy_patch_targets
...
def load_step36_config(path: str | Path) -> dict[str, Any]:
    with Path(path).open("r", encoding="utf-8") as handle:
        config = yaml.safe_load(handle)
    if not isinstance(config, dict):
        raise ValueError(f"Step36 config must be a mapping: {path}")
    if config.get("stage") != STAGE:
        raise ValueError(f"unexpected Step36 stage: {config.get('stage')!r}")
    return config


def missing_step36_inputs(config: dict[str, Any]) -> list[str]:
    required = [
    ...
    if not missing:
        step35_eval = _read_json(config["input"]["step35_eval_json"])
        if not bool(step35_eval.get("pass")):
            missing.append("Step35 eval did not pass")
    return missing


def load_step36_samples(config: dict[str, Any], required_ids_by_shard: dict[str, set[str]]) -> dict[str, Any]:
    """Load selected Step33B/Step32 artifacts into patch/token selector samples."""
    ...
    shard1 = load_patch_selector_samples(
    ...
    shard0 = load_patch_selector_samples(
    ...
    return {
        ...
        "summary": {
            ...
            "proxy_patch_target_shape": list(config["data"].get("proxy_patch_target_shape", [16, 392])),
        },
    }


def load_patch_selector_samples(
    *,
...
def load_patch_selector_sample(
    *,
    ...
    target = proxy_patch_targets(importance_artifact["context_importance_norm"]).squeeze(0).contiguous()
...
def make_step36_sample_from_tensors(
    *,
    sample_id: str,
    trajectory_id: str,
    shard_id: str,
    ...
    target = proxy_patch_target.detach().to(dtype=torch.float32, device="cpu")
    if target.ndim != 2:
        raise ValueError(f"proxy_patch_target must be [T, S], got {tuple(target.shape)}")
    if list(target.shape) != [int(context.shape[0]), int(context.shape[1])]:
        raise ValueError(f"proxy target shape {list(target.shape)} does not match context tokens {list(context.shape[:2])}")
    sample = {
        ...
        "proxy_patch_target": target.contiguous(),
        "metadata": {
            "future_tokens_exposed_to_selector": False,
            "action_used_as_input": False,
            "language_used_as_input": False,
    ...
    validate_step36_sample(sample)
    return sample


def validate_step36_sample(sample: dict[str, Any]) -> bool:
    context = sample.get("context_tokens")
    current = sample.get("current_tokens")
    target = sample.get("proxy_patch_target")
...
def batch_step36_samples(samples: list[dict[str, Any]], device: torch.device) -> dict[str, torch.Tensor]:
