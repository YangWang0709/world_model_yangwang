"""Dataset helpers for Step35 proxy-temporal selector smoke training."""
...
def load_step35_config(path: str | Path) -> dict[str, Any]:
    with Path(path).open("r", encoding="utf-8") as handle:
        config = yaml.safe_load(handle)
    if not isinstance(config, dict):
        raise ValueError(f"Step35 config must be a mapping: {path}")
    if config.get("stage") != STAGE:
        raise ValueError(f"unexpected Step35 stage: {config.get('stage')!r}")
    return config


def missing_step35_inputs(config: dict[str, Any]) -> list[str]:
    required = [
        ...
        config["input"]["step34_plan_summary_json"],
        config["input"]["step34_eval_json"],
    ...
    if not missing:
        step34_eval = _read_json(config["input"]["step34_eval_json"])
        if not bool(step34_eval.get("pass")):
            missing.append("Step34 eval did not pass")
    return missing


def load_step35_samples(config: dict[str, Any], required_ids_by_shard: dict[str, set[str]]) -> dict[str, Any]:
    """Load selected Step33B/Step32 artifacts into compact temporal features."""

    max_per_shard = int(config.get("data", {}).get("max_samples_per_shard", 64))
    shard1_ids = set(required_ids_by_shard.get("shard1", set()))
    shard0_ids = set(required_ids_by_shard.get("shard0", set()))
    shard1 = load_temporal_selector_samples(
        token_manifest_jsonl=config["input"]["token_manifest_jsonl"],
        importance_manifest_jsonl=config["input"]["importance_manifest_jsonl"],
        allowed_sample_ids=shard1_ids,
        shard_id="shard1",
    ...
    shard0 = load_temporal_selector_samples(
        ...
        allowed_sample_ids=shard0_ids & shard0_gap0_ids,
        shard_id="shard0",
...
def load_temporal_selector_samples(
    *,
    ...
    for sample_id in ordered_ids[: int(max_samples)]:
        ...
        sample = load_temporal_selector_sample(
...
def load_temporal_selector_sample(
    *,
    ...
    context_importance = importance_artifact["context_importance_norm"].detach().to(dtype=torch.float32, device="cpu")
    target = proxy_temporal_targets(context_importance).squeeze(0).contiguous()
...
def make_step35_sample_from_tensors(
    *,
    sample_id: str,
    trajectory_id: str,
    shard_id: str,
    ...
    context = context_tokens.detach().to(dtype=torch.float32, device="cpu")
    current = current_tokens.detach().to(dtype=torch.float32, device="cpu")
    if context.ndim != 3:
    ...
    target = proxy_temporal_target.detach().to(dtype=torch.float32, device="cpu").reshape(-1)
    if int(target.numel()) != int(context.shape[0]):
        raise ValueError(f"proxy target length {target.numel()} != context frames {context.shape[0]}")
    sample = {
        "sample_id": str(sample_id),
        "trajectory_id": str(trajectory_id),
        "shard_id": str(shard_id),
        "data_package_id": str(data_package_id),
        "context_temporal_features": context.mean(dim=1).contiguous(),
        "current_summary": current.mean(dim=(0, 1)).contiguous(),
        "proxy_temporal_target": target.contiguous(),
        "metadata": {
            "future_tokens_exposed_to_selector": False,
            "action_used_as_input": False,
            "language_used_as_input": False,
    ...
    validate_step35_sample(sample)
    return sample


def validate_step35_sample(sample: dict[str, Any]) -> bool:
    context = sample.get("context_temporal_features")
    current = sample.get("current_summary")
    target = sample.get("proxy_temporal_target")
    if not isinstance(context, torch.Tensor) or context.ndim != 2:
        raise ValueError("context_temporal_features must be [T, D]")
    if not isinstance(current, torch.Tensor) or current.ndim != 1:
        raise ValueError("current_summary must be [D]")
    if not isinstance(target, torch.Tensor) or target.ndim != 1:
        raise ValueError("proxy_temporal_target must be [T]")
    if int(target.shape[0]) != int(context.shape[0]):
        raise ValueError("target length must match context frames")
    for key, tensor in {
        "context_temporal_features": context,
...
def batch_step35_samples(samples: list[dict[str, Any]], device: torch.device) -> dict[str, torch.Tensor]:
    for sample in samples:
        validate_step35_sample(sample)
    return {
        "context_temporal_features": torch.stack([sample["context_temporal_features"] for sample in samples]).to(device),
        "current_summary": torch.stack([sample["current_summary"] for sample in samples]).to(device),
...
def sample_ids_by_shard_from_splits(config: dict[str, Any]) -> dict[str, set[str]]:
    from data.bridgedata_v2_proxy_temporal_selector_splits_step35 import build_step35_training_settings
