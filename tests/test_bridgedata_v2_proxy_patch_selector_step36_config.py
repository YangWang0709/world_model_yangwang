...
CONFIG = Path("configs/bridgedata_v2_tfds_proxy_patch_selector_train_step36.yaml")


def test_step36_config_exists_and_keeps_decision_flags_false():
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    assert config["stage"] == "bridgedata_v2_tfds_proxy_patch_selector_train_step36"
    assert config["selector"]["module"] == "ProxyPatchTokenSelectorHead"
    assert config["selector"]["patch_level_selector"] is True
    assert config["selector"]["temporal_only"] is False
...
def test_step36_config_forbids_forbidden_work_and_checkpoints():
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    ...
    assert config["training"]["allowed_optimizer_scopes"] == ["proxy_patch_token_selector_head_only"]
