...
CONFIG = Path("configs/bridgedata_v2_tfds_factorized_selector_step37.yaml")


def test_step37_config_exists_and_keeps_decision_flags_false():
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    assert config["stage"] == "bridgedata_v2_tfds_factorized_selector_step37"
    assert "factorized_temporal_spatial_selector" in config["selector_variants"]["run_variants"]
    assert "factorized_temporal_spatial_with_proxy_temporal_prior" in config["selector_variants"]["run_variants"]
    assert config["factorized_selector"]["final_selector"] is False
    assert config["factorized_selector"]["train_current_importance"] is False
...
def test_step37_config_forbids_forbidden_work_and_checkpoints():
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    ...
    for key in [
        ...
        "use_existing_step35_temporal_baseline",
        "use_existing_step36_patch_baseline",
    ...
    assert config["training"]["allowed_optimizer_scopes"] == [
        "proxy_factorized_selector_head_only",
        "proxy_selector_ablation_heads_only",
    ]
