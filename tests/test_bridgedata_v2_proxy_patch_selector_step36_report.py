...
def test_step36_eval_writes_reports_and_keeps_flags_false(tmp_path: Path):
    ...
    config["output"]["metrics_json"] = str(run / "patch_selector_metrics.json")
    config["output"]["gate_decision_json"] = str(run / "step36_gate_decision.json")
    config["output"]["eval_json"] = str(run / "step36_eval.json")
    config["output"]["eval_md"] = str(run / "step36_eval.md")
    run.mkdir()
    _write(
    ...
    metric = {
        ...
        "patch_selector_beats_current_only_patch_baseline": True,
        "patch_selector_spearman": 0.5,
        "patch_selector_pearson": 0.5,
        "top64_overlap": 0.2,
        "top128_overlap": 0.2,
        "top256_overlap": 0.2,
        "top512_overlap": 0.2,
    ...
    _write(
        ...
        {
            "future_patch_selector_gate_ready": True,
            "patch_selector_beats_random_token_baseline": True,
            "patch_selector_beats_uniform_token_baseline": True,
            "patch_selector_beats_temporal_broadcast_baseline": True,
            "patch_selector_beats_current_only_patch_baseline": True,
            "patch_selector_generalizes_cross_shard": True,
            "top256_overlap_mean": 0.2,
            "selector_training_allowed": False,
            "final_selector_training_allowed": False,
            "current_importance_training_allowed": False,
            "context_utility_claim_allowed": False,
            "recommended_step37": {"name": "next", "scope": "bounded"},
        },
    )
    config_path = tmp_path / "config.yaml"
    config_path.write_text(yaml.safe_dump(config), encoding="utf-8")

    summary = evaluate_step36_proxy_patch_selector(config_path)
    ...
    assert Path("docs/STEP36_PROXY_PATCH_SELECTOR_TRAINING_SMOKE.md").exists()
