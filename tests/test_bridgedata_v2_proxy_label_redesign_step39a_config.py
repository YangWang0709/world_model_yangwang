...
CONFIG = Path("configs/bridgedata_v2_tfds_proxy_label_redesign_step39a.yaml")


def test_step39a_config_has_safe_defaults():
    config = load_step39a_config(CONFIG)
    ...
    assert config["data"]["load_future_tokens_for_label_redesign"] is False
    ...
    assert config["guards"]["no_state_dict_save"] is True
...
def test_step39a_config_outputs_only_ignored_run_dir():
    config = yaml.safe_load(CONFIG.read_text(encoding="utf-8"))
    output_text = "\n".join(str(value) for value in config["output"].values())

    assert "runs/bridgedata_v2_tfds_proxy_label_redesign_step39a_v1" in output_text
