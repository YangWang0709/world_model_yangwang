...
from data.bridgedata_v2_proxy_patch_selector_dataset_step36 import make_step36_sample_from_tensors
from training.bridgedata_v2_proxy_patch_selector_trainer import _train_one_setting


def test_step36_trainer_updates_only_proxy_patch_selector_head():
    config = {
        "seed": 7,
        "selector": {
            "token_dim": 6,
    ...
    assert row["optimizer_scope"] == "proxy_patch_token_selector_head_only"
    ...
    assert row["patch_level_selector_training_performed"] is True
    assert row["checkpoint_saved"] is False
    assert curve["curve"]


def _sample(sample_id: str, trajectory_id: str):
    return make_step36_sample_from_tensors(
        sample_id=sample_id,
