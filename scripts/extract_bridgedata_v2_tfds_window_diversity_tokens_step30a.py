"""Run Step30A limited clip export and local-only VideoMAE token extraction."""
...
def extract_step30a_window_diversity_tokens(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    config = _load_yaml(config_path)
    missing = _missing_inputs(config)
    if missing:
        summary = _safe_stop_summary(config, f"missing Step30A token extraction inputs: {missing}")
    ...
    summary.update(
        {
            "stage": "bridgedata_v2_tfds_window_diversity_step30a_token_extraction",
...
def main() -> None:
    ...
    print(json.dumps(extract_step30a_window_diversity_tokens(args.config), indent=2, sort_keys=True))
