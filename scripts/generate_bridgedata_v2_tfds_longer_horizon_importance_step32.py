"""Generate Step32 limited proxy importance from longer-horizon token artifacts."""
...
from scripts.generate_bridgedata_v2_tfds_importance_dryrun import generate_bridgedata_v2_tfds_importance_from_config

DEFAULT_CONFIG = PROJECT_ROOT / "configs" / "bridgedata_v2_tfds_longer_horizon_step32.yaml"


def generate_step32_longer_horizon_importance(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    config = _load_yaml(config_path)
    missing = _missing_inputs(config)
    if missing:
        summary = _safe_stop_summary(config, f"missing Step32 importance inputs: {missing}")
    ...
    horizon_summary = summarize_longer_horizon_proxy_importance(
    ...
    summary.update(
        {
            "stage": "bridgedata_v2_tfds_longer_horizon_step32_importance",
...
def main() -> None:
    ...
    print(json.dumps(generate_step32_longer_horizon_importance(args.config), indent=2, sort_keys=True))
