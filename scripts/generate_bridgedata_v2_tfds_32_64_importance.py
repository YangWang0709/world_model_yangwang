"""Generate Step29 limited proxy importance from Step29 token artifacts."""
...
from data.bridgedata_v2_tfds_expanded_importance_manifest import (
    enrich_importance_manifest_with_split,
...
def generate_step29_importance(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    config = _load_yaml(config_path)
    missing = _missing_inputs(config)
    if missing:
        summary = _safe_stop_summary(config, f"missing Step29 importance inputs: {missing}")
    ...
    records = []
    if summary.get("importance_generation_performed"):
        records = enrich_importance_manifest_with_split(
            config["output"]["importance_manifest_jsonl"],
            config["output"]["split_json"],
        )
    split_summary = summarize_expanded_importance_manifest(records) if records else {}
    summary.update(
        {
            "stage": "bridgedata_v2_tfds_trainval_context_utility_step29_importance",
...
def main() -> None:
    ...
    print(json.dumps(generate_step29_importance(args.config), indent=2, sort_keys=True))
