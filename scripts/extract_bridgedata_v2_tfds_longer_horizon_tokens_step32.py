"""Run Step32 limited clip export and local-only VideoMAE token extraction."""
...
def extract_step32_longer_horizon_tokens(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    config = _load_yaml(config_path)
    missing = _missing_inputs(config)
    if missing:
        summary = _safe_stop_summary(config, f"missing Step32 token extraction inputs: {missing}")
        _write_json(Path(config["output"]["token_summary_json"]), summary)
        return summary

    windows = read_jsonl(config["output"]["horizon_window_manifest_jsonl"])
    ...
    horizons = sorted({int(record["horizon_gap"]) for record in windows})
    summary.update(
        {
            "stage": "bridgedata_v2_tfds_longer_horizon_step32_token_extraction",
...
def _adapter_config(config: dict[str, Any], selected_count: int) -> dict[str, Any]:
    ...
    adapter["input"]["resolved_window_manifest_jsonl"] = config["output"]["horizon_window_manifest_jsonl"]
...
def main() -> None:
    ...
    print(json.dumps(extract_step32_longer_horizon_tokens(args.config), indent=2, sort_keys=True))
