"""Smoke test Student selector training on real VideoMAE importance labels."""
...
from scripts.smoke_test_predictive_importance_real_video_videomae import run_real_videomae_importance_smoke
...
REPORT_PATH = PROJECT_ROOT / "docs" / "STUDENT_SELECTOR_REAL_VIDEOMAE_SMOKE_REPORT.md"
EXPECTED_RUN_DIR = PROJECT_ROOT / "runs" / "student_selector_real_video_videomae_smoke_v1"
...
def _write_report(summary: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        f"- token shard input dir: `{summary['token_shard_dir']}`",
        f"- importance shard input dir: `{summary['importance_shard_dir']}`",
        ...
        "```",
        "",
        f"STUDENT_SELECTOR_REAL_VIDEOMAE_SMOKE_PASS = {str(summary['pass']).lower()}",
        "",
    ]
...
def run_student_selector_real_videomae_smoke(config_path: str | Path = CONFIG_PATH) -> dict[str, Any]:
    config = load_yaml(config_path)
    token_dir = Path(config["data"]["token_shard_dir"])
    importance_dir = Path(config["data"]["importance_shard_dir"])
    token_glob = str(config["data"].get("token_shard_glob", "tokens_shard_*.pt"))
    importance_glob = str(config["data"].get("importance_shard_glob", "importance_shard_*.pt"))
    if not list(token_dir.glob(token_glob)):
        raise FileNotFoundError(
            f"Missing real VideoMAE token shards in {token_dir}; Step 10C does not rerun extraction."
        )
    if not list(importance_dir.glob(importance_glob)):
        run_real_videomae_importance_smoke()
    if not list(importance_dir.glob(importance_glob)):
        raise FileNotFoundError(f"Missing real VideoMAE importance shards in {importance_dir}")
    ...
    checks = {
        ...
        "loss_decreased_or_warn": bool(training_summary),
        "final_importance_mse_finite": bool(
            training_summary and _finite(training_summary.get("final_importance_mse"))
        ),
        "final_importance_mae_finite": bool(
            training_summary and _finite(training_summary.get("final_importance_mae"))
        ),
        "final_pearson_corr_finite": bool(
            training_summary and _finite(training_summary.get("final_pearson_corr_mean"))
        ),
        "final_target_topk_overlap_finite": bool(
            training_summary and _finite(training_summary.get("final_target_topk_overlap"))
        ),
        "final_target_topk_overlap_positive": bool(
            training_summary and float(training_summary.get("final_target_topk_overlap", 0.0)) > 0.0
        ),
    ...
    pass_keys = [key for key in checks if key not in {"loss_decreased"}]
    pass_flag = all(bool(checks[key]) for key in pass_keys)
    if training_summary:
    ...
    cloud_recommendation = (
        ...
        else "not required for Step 10C; local RTX 5080 smoke succeeded"
    )
    summary = {
        "token_shard_dir": str(token_dir),
        "importance_shard_dir": str(importance_dir),
        "run_dir": str(run_dir),
        "checkpoint_path": checkpoint_path,
        "num_samples": training_summary.get("num_samples") if training_summary else None,
...
def main() -> None:
    summary = run_student_selector_real_videomae_smoke()
    print(json.dumps(summary, indent=2))
    print(f"STUDENT_SELECTOR_REAL_VIDEOMAE_SMOKE_PASS = {str(summary['pass']).lower()}")
    if not summary["pass"]:
        raise SystemExit(1)
