"""Generate Step33B limited proxy-importance artifacts."""
...
def generate_step33b_data_diversity_importance(config_path: str | Path = DEFAULT_CONFIG) -> dict[str, Any]:
    os.environ.setdefault("TRANSFORMERS_OFFLINE", "1")
    os.environ.setdefault("HF_HUB_OFFLINE", "1")
    config = _load_yaml(config_path)
    missing = _missing_inputs(config)
    if missing:
        summary = _safe_stop_summary(config, f"missing Step33B importance inputs: {missing}")
    ...
    distribution = summarize_step33b_proxy_importance(
    ...
    summary.update(
        {
            "stage": "bridgedata_v2_tfds_data_diversity_step33b_importance",
...
def main() -> None:
    ...
    print(json.dumps(generate_step33b_data_diversity_importance(args.config), indent=2, sort_keys=True))
